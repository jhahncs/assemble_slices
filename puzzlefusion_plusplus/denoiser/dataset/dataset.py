    ...
    def __getitem__(self, idx):
        ...
        for i in range(num_parts):
            ...
            if normal_vector is None:
                ...
                #print(normal_vector)
            pc, gt_trans, gt_quat = rotate_and_translate_to_xy_plane(pc, normal_vector)
        ...
        print(f'\npart_trans_ref_{part_trans_ref.shape}_{part_trans_ref}\n')
        print(f'\npart_rots_ref_{part_rots_ref.shape}_{part_rots_ref}\n')
        # random perturb the reference part
        noise_trans, noise_rots = get_noise_trans_rots(part_trans_ref.shape, part_rots_ref.shape)

        
        timesteps = torch.randint(0, 50, (1,)).long()
        #print(f'\nnoise_trans_{noise_trans.shape}_{noise_trans.cpu().numpy()}\n')
        #print(f'\nnoise_rots_{noise_rots.shape}_{noise_rots.cpu().numpy()}\n')
        ...
        #data_dict['noise_trans'] = noise_trans
        #data_dict['noise_rots'] = noise_rots
        return data_dict
